...
import { assertRoleAssignment } from "@/lib/admin-role-policy";
...
export async function GET(request: NextRequest) {
  ...
  const staff = await query<{
    ...
    "SELECT s.id, s.username, s.display_name displayName, s.email, s.phone, s.status, s.role_id roleId, r.code roleCode, r.name_th roleNameTh, r.name_en roleNameEn, s.created_at createdAt, COALESCE(GROUP_CONCAT(p.code ORDER BY p.code SEPARATOR ','), '') permissionCodes FROM admin_staff s JOIN admin_roles r ON r.id = s.role_id LEFT JOIN admin_role_permissions rp ON rp.role_id = r.id LEFT JOIN admin_permissions p ON p.id = rp.permission_id WHERE s.status <> 'deleted' GROUP BY s.id, s.username, s.display_name, s.email, s.phone, s.status, s.role_id, r.code, r.name_th, r.name_en, s.created_at ORDER BY s.id DESC",
  );
  return NextResponse.json({ staff: staff.map((admin) => ({ ...admin, permissionCodes: admin.permissionCodes ? admin.permissionCodes.split(",") : [] })) });
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    body = await parseJsonBody(request, staffSchema.extend({ password: z.string().min(10).max(200) }));
  } catch (error) {
    return validationErrorResponse(error);
  }
  const passwordHash = await hashPassword(body.password);
  const roleError = await assertRoleAssignment(request, body.roleId);
...
export async function PUT(request: NextRequest) {
  ...
  const roleError = await assertRoleAssignment(request, body.roleId, body.id);
  if (roleError) return roleError;
  if (body.password) {
    const passwordHash = await hashPassword(body.password);
    await query(
...
export async function DELETE(request: NextRequest) {
  ...
  const [target] = await query<{ roleId: number }>("SELECT role_id roleId FROM admin_staff WHERE id = ?", [id]);
  if (!target) return NextResponse.json({ message: "Staff not found" }, { status: 404 });
  const roleError = await assertRoleAssignment(request, target.roleId, id);
  if (roleError) return roleError;
